...
class PurpleCreatePropertyQuickFix extends BaseIntentionAction {

    private final String key;

    PurpleCreatePropertyQuickFix(String key) {
        this.key = key;
    }
        ...
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            if (virtualFiles.size() == 1) {
                ...
                final FileChooserDescriptor descriptor =
                        FileChooserDescriptorFactory.createSingleFileDescriptor(PurpleFileType.INSTANCE);
    ...
    private void createProperty(final Project project, final VirtualFile file) {
        WriteCommandAction.writeCommandAction(project).run(() -> {
            PurpleFile simpleFile = (PurpleFile) PsiManager.getInstance(project).findFile(file);
            ...
            if (lastChildNode != null/* && !lastChildNode.getElementType().equals(SimpleTypes.CRLF)*/) {
                simpleFile.getNode().addChild(PurpleElementFactory.createCRLF(project).getNode());
            }
            // IMPORTANT: change spaces to escaped spaces or the new node will only have the first word for the key
            PurpleProperty property = PurpleElementFactory.createProperty(project, key.replaceAll(" ", "\\\\ "), "");
