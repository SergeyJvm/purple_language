...
public class PurpleSpellcheckingStrategy extends SpellcheckingStrategy {

    @Override
    public @NotNull
    Tokenizer<?> getTokenizer(PsiElement element) {
        if (element instanceof PsiComment) {
            return new PurpleCommentTokenizer();
        }

        if (element instanceof PurpleProperty) {
            return new PurplePropertyTokenizer();
        }

        return EMPTY_TOKENIZER;
    }

    private static class PurpleCommentTokenizer extends Tokenizer<PsiComment> {
    ...
    private static class PurplePropertyTokenizer extends Tokenizer<PurpleProperty> {

        public void tokenize(@NotNull PurpleProperty element, TokenConsumer consumer) {
            //Spell check the keys and values of properties with different splitters
            final ASTNode key = element.getNode().findChildByType(PurpleTypes.KEY);
            ...
            final ASTNode value = element.getNode().findChildByType(PurpleTypes.VALUE);
